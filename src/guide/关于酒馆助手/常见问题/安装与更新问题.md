...
- 升级酒馆版本，需要1.12.12以上酒馆

## 更新或卸载提示成功, 但安装新版后仍显示旧版

**问题**: 更新版本提示成功了, 或者直接卸载扩展安装新版, 但仍显示旧版

**解决**: 酒馆可能存在未卸载干净的情况, 建议多次执行卸载、刷新的操作, 或者直接[手动删除扩展](./安装与更新问题#提示文件夹已存在)

## 更新前正常使用, 更新后扩展异常

**问题**: 更新前正常使用, 更新后出现以下问题：
 - 扩展面板显示不完整
 - 弹出红色报错弹窗：`Error rendering template: Check the DevTools console for more information.`
 - 新版的特性未生效

**解决**: 
 - 清理浏览器缓存（包括 Cookie 和网站数据也要清理, 如果你知道怎么操作, 也可以只清理酒馆的特定数据）
 - 如果使用了 cloudflare、nginx 等反向代理, 请调整缓存设置
...
- [`Extension installation failed: Server Error: Cloning into 'data/default-user/extensions/JS-Slash-Runner'... warning:url contains a newline in its path component`](#链接输入错误)
...
::: tip
如果上方的路径找不到文件夹, 或者删除上方路径后酒馆助手依然存在, 请查看`SillyTavern/public/scripts/extensions/third-party`下是否也有`JS-Slash-Runner`
:::
...
- 怎么都找不到文件夹但还是提示存在? **终极方案**:
  ...
  **尽量直接复制粘贴命令, 避免漏掉空格或者中英文符号混用**
  
  1. 打开Termux, 如果使用了一键脚本, 输入 `8` 退出（如果是其他一键脚本, 参考其提供的退出方式）
  2. 输入下面的内容, 回车

  ```bash
  cd /
  ```
  此时前缀会变成 `root@localhost:/#`或类似内容, 在此行继续后面的操作
  ...
  :::warning
  此方法删除后不会有任何提示语, 只会另起一空行显示, 请谨慎使用. 下面是直接一键删除的命令, 但可控性较差:
...
1. 启动 Termux
2. 如果使用了一键脚本, 输入 `8` 退出（如果是其他一键脚本, 参考其提供的退出方式）
3. 发送以下内容更新包管理器
...
- 黄色警告弹窗提示 `Extension installation failed: Server Error: Cloning into 'data/default-user/extensions/JS-Slash- Runner'... warning:url contains a newline in its path component`
