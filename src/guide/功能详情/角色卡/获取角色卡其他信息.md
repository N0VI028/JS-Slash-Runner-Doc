...
**这是还没调整的旧函数, 可能接口并不怎么方便, 等后续重新做.**

:::tip
请务必先阅读[如何正确使用酒馆助手](/guide/基本用法/如何正确使用酒馆助手.md)
:::

[点击查看对应类型定义文件 (可发给 AI 或 IDE 使用, 酒馆助手界面中提供了打包下载)](https://github.com/N0VI028/JS-Slash-Runner/blob/main/%40types/function/raw_character.d.ts)
...
```ts [getCharData]
function getCharData(name: LiteralUnion<'current' | string>): Promise<v1CharData | null>;
```
...
```ts
function getCharAvatarPath(name: LiteralUnion<'current' | string>): Promise<string | null>;
```
...
function getChatHistoryBrief(name: LiteralUnion<'current' | string>): Promise<ChatHistoryBrief[] | null>;
```
